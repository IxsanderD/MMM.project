...
# We see that both Yee and FCI produce the correct frequency seprations, validating the set-up. We can now do some more exotic measurements.
# We can for example add another layer to our etalon and study this effect. We do this for a d=20 layer.
...
# solver.add_source(xs,ys,J0,tc,width,Wc)
# solver.add_recorder(xr,yr)
# solver.add_material([Nx-N_PML-d//2-d//2,Nx-N_PML-3*d//2-d//2],[Nx-N_PML-d//2+d//2,Nx-N_PML-3*d//2+d//2],[0,0],[Ny,Ny],eps_r=[n2**2,n1**2],mu_r=[1,1],sigma=[0,0])
# solver.update_loop()
# # solver.show_recorder()
...
# plt.plot(f[mask]*10**9,R[mask],color='green')
